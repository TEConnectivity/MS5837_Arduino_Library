...
*/
boolean ms5837::is_connected(void) {
  Wire.beginTransmission((uint8_t)MS5837_ADDR);
...
*/
enum ms5837_status ms5837::write_command(uint8_t cmd) {
  uint8_t i2c_status;
  ...
  /* Do the transfer */
  if (i2c_status == ms5837_STATUS_ERR_OVERFLOW)
    return ms5837_status_no_i2c_acknowledge;
  if (i2c_status != ms5837_STATUS_OK)
...
*
*/
void ms5837::set_resolution(enum ms5837_resolution_osr res) {
...
*       - ms5837_status_no_i2c_acknowledge : I2C did not acknowledge
*/
enum ms5837_status ms5837::reset(void) {
  return write_command(MS5837_RESET_COMMAND);
}
...
*/
enum ms5837_status ms5837::read_eeprom_coeff(uint8_t command, uint16_t *coeff) {
  uint8_t buffer[2];
  uint8_t i;
  uint8_t i2c_status;
  ...
  Wire.requestFrom((uint8_t)MS5837_ADDR, 2U);
  for (i = 0; i < 2; i++) {
    buffer[i] = Wire.read();
  }
...
*       - ms5837_status_crc_error : CRC check error on the coefficients
*/
enum ms5837_status ms5837::read_eeprom(void) {
  enum ms5837_status status;
  uint8_t i;

  for (i = 0; i < MS5837_COEFFICIENT_COUNT; i++) {
    status = read_eeprom_coeff(MS5837_PROM_ADDRESS_READ_ADDRESS_0 + i * 2,
                               eeprom_coeff + i);
    if (status != ms5837_status_ok)
      return status;
  }
  if (!crc_check(eeprom_coeff, (eeprom_coeff[MS5837_CRC_INDEX] & 0xF000) >> 12))
...
*/
boolean ms5837::crc_check(uint16_t *n_prom, uint8_t crc) {
  uint8_t cnt, n_bit;
...
*       - ms5837_status_no_i2c_acknowledge : I2C did not acknowledge
*/
enum ms5837_status ms5837::conversion_and_read_adc(uint8_t cmd, uint32_t *adc) {
  enum ms5837_status status;
  uint8_t i2c_status;
  uint8_t buffer[3];
  uint8_t i;
  ...
  delay(conversion_time[(cmd & MS5837_CONVERSION_OSR_MASK) / 2]);

  Wire.beginTransmission((uint8_t)MS5837_ADDR);
  Wire.write((uint8_t)0x00);
  i2c_status = Wire.endTransmission();

  Wire.requestFrom((uint8_t)MS5837_ADDR, 3U);
  for (i = 0; i < 3; i++) {
    buffer[i] = Wire.read();
  }
...
*       - ms5837_status_crc_error : CRC check error on the coefficients
*/
enum ms5837_status ms5837::read_temperature_and_pressure(float *temperature,
                                                         float *pressure) {
  ...
  // OFF = OFF_T1 + TCO * dT
  OFF = ((int64_t)(eeprom_coeff[MS5837_PRESSURE_OFFSET_INDEX]) << 17) +
  ...
  SENS =
      ((int64_t)eeprom_coeff[MS5837_PRESSURE_SENSITIVITY_INDEX] << 16) +
